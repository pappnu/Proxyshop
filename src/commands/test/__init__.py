...
@click.option(
    "-T", "--target", is_flag=True, default=False, help="Evaluate a specific test case."
)
def test_frame_logic(target: bool = False):
    ...
    while True:
        choice = input(
            "Enter the number for a test case to evaluate:\n"
...
@click.group(
    ...
    commands={"logic.frame": test_frame_logic, "logic.text": test_text_logic},
)
def test_cli():
