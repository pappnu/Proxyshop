...
@click.group(name="compress", help="Command utilities for compressing files.")
def compress_cli():
    """File utilities CLI."""
    pass
...
@compress_cli.command(
    name="template", help="Compress a Photoshop template file (PSD/PSB)."
)
...
def compress_template(template: str, plugin: str | None = None) -> None:
    ...
    if not path.is_file():
        print(
            f"I couldn't find a template named '{template}' at this path:\n{str(path)}"
        )
        return
    compress_7z(path)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Compress built-in plugins as well.",
)
def compress_all(plugins: bool = False) -> None:
    ...
    if plugins:
        plugins = [
            Path(PATH.PLUGINS, p, "templates") for p in ["Investigamer", "SilvanMTG"]
        ]
